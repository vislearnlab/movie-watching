...
- Step 2: Export the Round 1 annotations as a CSV file and add to the `data/segmentation/round1` folder here. Then, run `prepare_stimuli_videos_datavyu.ipynb` which will create a new file `stimuli_prompts_round1.csv` that is ready to be inputted into `SAM`.

- Step 3: Run `annotate_videos.py --csv data/stimuli_prompts_round1.csv --output preprocessing/segmentation/output_round1 --render_video --skip_existing --debug points` in a [tmux](https://www.redhat.com/en/blog/introduction-tmux-linux) window. This selects all GPUs by default. You can specify certain GPUs by using `--gpu 0 1` for example. Check available GPUs in Tversky by running `nvidia-smi` This step tries to annotate SAM on all of the videos with the raw annotations.
...
- Step 5: Export the Round 2 annotations and add to the `data/segmentation/round2` folder here. Then, set `CURRENT_ROUND` to be `round2` at the top of `prepare_stimuli_videos_datavyu.ipynb` before running it, which will create a new file `stimuli_prompts_round2.csv` that is ready to be inputted into `SAM` again. 

- Step 6: Run `annotate_videos.py --csv data/stimuli_prompts_round2.csv --output preprocessing/segmentation/output_round2 --render_video --skip_existing --debug points`
